...
function readOrgClaims(identity: Record<string, unknown>): {
  ...
  const orgObj = identity.o as Record<string, string> | undefined;
  const orgId = typeof identity.org_id === "string" ? identity.org_id : orgObj?.id || null;
  const orgRole =
    typeof identity.org_role === "string" ? identity.org_role : orgObj?.rol || null;
  const orgSlug =
    typeof identity.org_slug === "string" ? identity.org_slug : orgObj?.slg || null;
