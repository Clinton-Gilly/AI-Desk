...
export const whoami = query({
  ...
  handler: async (ctx) => {
    ...
    console.log("CLERK JWT CLAIMS:", claims);
    
    const orgObj = claims.o as Record<string, string> | undefined;
    const orgId = typeof claims.org_id === "string" ? claims.org_id : orgObj?.id || null;
    const orgRole =
      typeof claims.org_role === "string" ? claims.org_role : orgObj?.rol || null;
    const orgSlug =
      typeof claims.org_slug === "string" ? claims.org_slug : orgObj?.slg || null;
