...
// ─────────────────────────────────────────────────────────────────────────────
// Public marketing landing page for MyChat — an AI customer-support desk.
// Lives at the app root, OUTSIDE the (app) Clerk group and the /widget route, so
// it stays Clerk-free (root layout has no ClerkProvider). The hero + reveals are
// client components (motion); the rest is server-rendered.
// ─────────────────────────────────────────────────────────────────────────────
            ...
            <div className="group relative flex h-full flex-col items-start gap-6 overflow-hidden rounded-2xl border border-border bg-card p-6 shadow-card transition-all duration-300 hover:border-brand/30 hover:shadow-elevated sm:flex-row sm:items-center sm:p-8">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1.5 max-w-2xl text-muted-foreground">
                  ...
                  the conversations that need a person — with the full thread and
              ...
              <div className="flex -space-x-2">
                {["from-neutral-200 to-neutral-400", "from-neutral-400 to-neutral-600", "from-neutral-600 to-neutral-800"].map(
                  (c, idx) => (
                    <span
                      key={idx}
                      className={`flex size-9 items-center justify-center rounded-full bg-gradient-to-br ${c} text-xs font-semibold text-white ring-2 ring-card`}
                    >
                      {["AI", "SR", "MJ"][idx]}
                    </span>
                  ),
